    ...
    <main>
        <?php if (isset($_GET['for']) && is_numeric($_GET['for'])): ?>
            <section class="caution">
                <p>Вы редактируете словарь пользователя <i><?= $user->username ?></i></p>
            </section>
        <?php endif ?>
            ...
            <section>
                <section class="langdict-options">
                    <div>
                        ...
                        <select id="lo_langselect" data-for="<?php if (isset($_GET['for']) && is_numeric($_GET['for'])) { echo $_GET['for']; } ?>">
                    ...
                    <div>
                        ...
                        <select id="lo_tagselect" data-for="<?php if (isset($_GET['for']) && is_numeric($_GET['for'])) { echo $_GET['for']; } ?>">
                            <option value=""></option>
                ...
                <div class="language-title">
                    ...
                    <div class="buttons">
                        <a href="/personal/add?langdict=<?= $currentLanguage->languageCode ?><?php if (isset($_GET['for']) && is_numeric($_GET['for'])) { echo "&for={$_GET['for']}"; } ?>" class="a-button">Добавить</a>
                        <?php if (!isset($_GET['for'])): ?>
                           <a href="/personal/train?langdict=<?= $currentLanguage->languageCode ?>" class="a-button">Тренироваться</a> 
                        <?php endif ?>
                        <a href="/personal/stats?langdict=<?= $currentLanguage->languageCode ?><?php if (isset($_GET['for']) && is_numeric($_GET['for'])) { echo "&for={$_GET['for']}"; } ?>" class="a-button">Статистика</a>
                    </div>
                </div>
                <?php if (!$words): ?>
                    ...
                    <table>
                        ...
                        <?php foreach($words as $word): ?>
                            <tr id="word<?= $word->wordID ?>">
                                <td><a href="/personal/word?langdict=<?= $currentLanguage->languageCode ?>&id=<?= $word->wordID ?><?php if (isset($_GET['for']) && is_numeric($_GET['for'])) { echo "&for={$_GET['for']}"; } ?>" target="_blank"><?= capitalizeFirstLetter($word->word) ?></a></td>
            ...
            <section>
                    ...
                    <section>
                        ...
                        <?php foreach ($allUserLanguages as $userLanguage): ?>
                            <div class="language-choice">
                                <a href="/personal?langdict=<?= $userLanguage->languageCode ?><?php if (isset($_GET['for']) && is_numeric($_GET['for'])) { echo "&for={$_GET['for']}"; } ?>"><?= $userLanguage->languageName ?></a>
                            </div>
                        <?php endforeach; ?>
                    </section>
                <?php endif; ?>
                <section>
                    ...
                    <div id="allLangsContainer">
                        <?php foreach ($allLanguages as $language): ?>
                            <div class="language-choice">
                                <a href="/personal?langdict=<?= $language->languageCode ?><?php if (isset($_GET['for']) && is_numeric($_GET['for'])) { echo "&for={$_GET['for']}"; } ?>"><?= $language->languageName ?></a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </section>
